...
class FlextDbtLdapClientMixin(FlextDbtLdapServiceBase):
    ...
    def extract_ldap_entries(
        ...
        def _run_extract_ldap_entries() -> p.Result[
            ...
            logger.info(
                ...
                search_base or settings.ldap_base_dn,
                search_filter,
            )
            result = self._search_entries_sync(
                base_dn=search_base or settings.ldap_base_dn,
    ...
    def validate_ldap_data(
        ...
        def _run_validate_ldap_data() -> p.Result[m.DbtLdap.ValidationMetrics]:
            ...
            required_attributes = settings.required_attributes
            ...
            metrics = m.DbtLdap.ValidationMetrics(
                ...
                validation_passed=quality_score >= settings.min_quality_threshold,
            )
            logger.info(
            ...
            if not metrics.validation_passed:
                return r[m.DbtLdap.ValidationMetrics].fail(
                    f"Data quality below threshold: {quality_score} < {settings.min_quality_threshold}",
                )
    ...
    def _map_entry_attributes(
        ...
        for ldap_attr, dbt_attr in settings.ldap_attribute_mapping.items():
        ...
        for attr, values in entry.items():
            if attr not in settings.ldap_attribute_mapping and attr != dn_attr:
    ...
    def _prepare_ldap_data_for_dbt(
        ...
        for schema_name, table_name in settings.ldap_schema_mapping.items():
            schema_entries = [
...
__all__: list[str] = ["FlextDbtLdapClientMixin"]
