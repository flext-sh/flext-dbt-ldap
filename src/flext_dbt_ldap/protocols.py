...
class FlextDbtLdapProtocols(FlextProtocols):
    """DBT LDAP protocols extending FlextProtocols with DBT LDAP-specific interfaces.
    ...
    @runtime_checkable
    class LdapIntegrationProtocol(FlextProtocols.Domain.Service, Protocol):
        """Protocol for LDAP data integration operations."""

        def extract_ldap_data(
            self, ldap_config: dict[str, object], extraction_config: dict[str, object]
    ...
    class QualityProtocol(FlextProtocols.Domain.Service, Protocol):
        ...
        def validate_ldap_schema_compliance(
            self, ldap_data: list[dict[str, object]], schema_rules: dict[str, object]
        ) -> FlextResult[dict[str, object]]:
    ...
    class PerformanceProtocol(FlextProtocols.Domain.Service, Protocol):
        ...
        def cache_ldap_extractions(
            self, extraction_config: dict[str, object], cache_config: dict[str, object]
        ) -> FlextResult[bool]:
